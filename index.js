...
app.post('/updateGood', urlencodedParser, function (req, res) {
  ...
  if (updateGoodsData.goodsId) {    
   var updateGoodsDataBasedOnIdQuery= "UPDATE goods SET goodsName = ?, goodsSize = ?,goodsDesc =?,goodsUnitOfMeasurement= ? WHERE goodsId = ?";
   con.query(updateGoodsDataBasedOnIdQuery,[updateGoodsData.goodsName,updateGoodsData.goodsSize,updateGoodsData.goodsDesc,updateGoodsData.goodsUnitOfMeasurement,updateGoodsData.goodsId],function(err, result){
    if (err) res.status(400).send(err);
...
app.post('/deleteGood', urlencodedParser, function (req, res) {
  ...
  if (req.body.goodsId) {
    ...
    var deleteGoodQuery = "UPDATE goods SET goodsStatus='INACTIVE' WHERE goodsId = ?";
...
app.post('/addStock', urlencodedParser, function (req, res) {
  ...
  if (req.body) {
    ...
    for (var i = 0; i < jsonData.length; i++) {
      addStockData.push([jsonData[i].goodsId, jsonData[i].billNo, jsonData[i].purchaseDateTime, jsonData[i].goodsActualPrice, jsonData[i].goodsSellingPrice, jsonData[i].goodsQuantity,'ACTIVE','NO']);
      //updateAvailableStock.push({"idVal":jsonData[i].goodsId,"qty":jsonData[i].goodsQuantity});      
    }
    async.forEachOf(jsonData, function (dataElement, i, inner_callback) {
    ...
    var addStockQuery = 'INSERT INTO stock (goodsId,billNo,purchaseDateTime,goodsActualPrice,goodsSellingPrice,stockCount,stockStatus,stockUtilized) VALUES ?';
...
app.post('/updateStock',urlencodedParser,function(req,res){
  ...
  if(req.body){
    ...
    if(updateStockJson.stockCount){
      ...
      var updateAvailableStockQuery = "CALL updateAvailableStock(?,?)";
      con.query(updateAvailableStockQuery, [stockCountValue, goodsIdValue], function (err, result) {
        if(err)console.log(err);
        res.status(200);
    ...
    // update stock table
    var updateStockQuery="UPDATE stock SET goodsId=?,billNo=?,purchaseDateTime=?,goodsActualPrice=?,goodsSellingPrice=?,stockCount=? WHERE stockId=?";
    con.query(updateStockQuery,[updateStockJson.goodsId,updateStockJson.billNo,updateStockJson.purchaseDateTime,updateStockJson.goodsActualPrice,updateStockJson.goodsSellingPrice,updateStockJson.stockCount,updateStockJson.stockId],function(err,result){
      if(err)console.log(err);
      res.status(200);
...
app.post('/deleteStock',urlencodedParser,function(req,res){
  ...
  if(req.body){
    ...
    con.query(reduceStockProcedureQuery,[stockIdValue],function(err,result){
      if(err)console.log(err);
      res.status(200);
      res.statusMessage="Available Stock updated";      
      res.end("reduced available stock from goods table")
    })
    var deleteStockQuery="UPDATE stock SET stockStatus='INACTIVE' WHERE stockId=?";
...
app.get('/listAllSales', urlencodedParser, function (req, res) {
  ...
  if (req.query.startDate && req.query.endDate) {
    ...
    con.query(listSalesProcedureQuery, [startDateValue, endDateValue], function (err, result) {
      //console.log(result);
      if (err) res.status(400).send(err);
      res.status(200).send(result);
    })    
...
app.post('/addSales', urlencodedParser, function (req, res) {
  ...
  if (req.body) {
    ...
    var addSalesData = [];
    for (var i = 0; i < jsonData.length; i++) {
      addSalesData.push([jsonData[i].billNo,jsonData[i].goodsId, jsonData[i].buyerName, jsonData[i].salesDate, jsonData[i].salesPrice, jsonData[i].salesQuantity, jsonData[i].salesAmount, jsonData[i].profitAmount,'ACTIVE']);
    }
    async.forEachOf(jsonData, function (dataElement, i, inner_callback) {
      var qty = Number(jsonData[i].salesQuantity);
      var Id = jsonData[i].goodsId
      var updateAvailableStocknSalesQuery = "CALL updateAvailableStocknSales(?,?)";
      con.query(updateAvailableStocknSalesQuery, [qty, Id], function (err, result) {
        if (err) {
          inner_callback(err);
        }
        else {
    ...
    async.forEachOf(jsonData, function (dataElement, i, inner_callback) {
      var qty = Number(jsonData[i].salesQuantity);
      var Id = jsonData[i].goodsId
      var updateStockUtlizedOnSaleQuery = "CALL updateStockUtilizedOnSale(?,?)";
      con.query(updateStockUtlizedOnSaleQuery, [qty, Id], function (err, result) {
        if (err) {
          inner_callback(err);
        }
        else {
    ...
    // add sales
    var addSalesQuery = 'INSERT INTO sales (billNo,goodsId,buyerName,salesDate,salesPrice,salesQuantity,salesAmount,profitAmount,salesStatus) VALUES ?';
...
app.post('/updateSale',urlencodedParser,function(req,res){
  ...
  if(req.body) {
    ...
    var updateAvailableStockonSalesUpdateQuery= "CALL updateAvailableStockonSalesUpdate(?,?)";
    con.query(updateAvailableStockonSalesUpdateQuery,[goodsIdValue,salesQuantity],function(err,result){
      if(err)console.log(err);
      res.status(200);
      res.statusMessage="updated sales";
      res.end("updated available stock and qtysold");
    })
    var updateAvailableStockonSalesUpdateQuery= "CALL updateStockUtilizedOnUpdateSale(?,?)";
    con.query(updateAvailableStockonSalesUpdateQuery,[salesQuantity,goodsIdValue],function(err,result){
      if(err)console.log(err);
      res.status(200);
      res.statusMessage="updated stock utilized";
      res.end("updated stock utilized");
    })
    var updateSaleQuery="UPDATE sales SET salesDate=?,salesPrice=?,salesQuantity=?,salesAmount=?,profitAmount=?,salesTotalAmout=? WHERE salesId=?";
    con.query(updateSaleQuery,[req.body.salesDate,req.body.salesPrice,req.body.salesQuantity,req.body.salesAmount,req.body.profitAmount,req.body.salesTotalAmout,req.body.salesId],function(err,result){
      if(err)console.log(err);
      res.status(200);
      res.statusMessage="updated sales";
...
app.post('/deleteSale',urlencodedParser,function(req,res){
  ...
  var deleteSaleQuery="UPDATE sales SET salesStatus='INACTIVE' where salesId='?'";
